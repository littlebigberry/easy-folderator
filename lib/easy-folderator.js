...
function getFinalPackLevel (pack, fileParts) {
	var packLevel = pack;
...
function removeFileExtension (file) {
	var sliceIndex = file.lastIndexOf('.') - file.length;
	return file.slice(0, sliceIndex);
}

function propertyizeFileName (file, options) {
	var name = removeFileExtension(file);
	var propertyName = S(name).camelize().s;
	if (options.captializeModules)
...
function packageJSFile (file, pack, options) {
	...
	var packLevel = getFinalPackLevel(pack, fileParts);

	var propertyName = propertyizeFileName(fileName, options);
	var fullPath = path.join(options.rootDir, file);
	packLevel[propertyName] = require(fullPath);
}

function packageStringFile (file, pack, options) {
	var fileParts = file.split('/');
	var fileName = fileParts.pop();
	...
	var propertyName = propertyizeFileName(fileName, options);
	var fullPath = path.join(options.rootDir, file);
	packLevel[propertyName] = fs.readFileSync(fullPath, 'utf8');
}
						...
						function (file, stat, callback) {
							...
							if (!stat.isDirectory()) {
								if (file.substr(-3) === '.js') {
									...
									packageStringFile(file, pack, options);
								}
							} else {
